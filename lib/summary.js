...
function formatSize(size, total, neg = false) {
  if (neg) {
...
function pushPackageRow(pkgs, analysis, table, trunc, options) {
  pkgs.forEach((pkg) => {
    table.push([
  ...
  const sumSize = pkgs.reduce((acc, pkg) => acc + pkg.size, 0);
  const sumNumFiles = pkgs.reduce((acc, pkg) => acc + pkg.files.length, 0);
  table.push([
    yellow(sumTitle),
...
function pushPackageDeltaRow(pkgs, analysis, table) {
  pkgs.forEach((pkg) => {
    if (pkg.removed) {
      table.push([
        `${pkg.name}@${pkg.packageJson.version}`,
        formatSize(pkg.size, analysis.metrics.baseline.size, true),
        `-${pkg.files.length}`
      ]);
    } else {
      table.push([
        `${pkg.name}@${pkg.packageJson.version}`,
        formatSize(pkg.size, analysis.metrics.baseline.size),
        `${pkg.files.length}`
      ]);
    }
  });
...
function summary(analysis, options) {
  ...
  table.push([
    red(`PACKAGE (${analysis.metrics.numPkgs})`),
    red(`SIZE (${formatSizeWithUnit(analysis.metrics.size)})`),
    red(`NUM OF FILES (${analysis.metrics.numFiles})`)
  ]);

  if ('baseline' in analysis.metrics) {
    pushPackageDeltaRow(sorted, analysis, table);
  } else {
    pushPackageRow(sorted, analysis, table, trunc, options);
  }
